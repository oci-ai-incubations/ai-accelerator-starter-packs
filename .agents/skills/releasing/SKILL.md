...
gh release create $VERSION \
  ...
  release_test_matrix/${VERSION}_dox_pack.zip \
...
```bash
cat ~/.Codex/settings.json | grep -o 'CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS[^,]*'
```

If not set, ask the user to add it to `~/.Codex/settings.json`:
```json
...
Teammates are full interactive Codex sessions. They will ask questions via `AskUserQuestion` when they need input (OCI login, compartment selection, etc.).
...
| dox_pack | small | <region>       | Track 3 | PASS   | —          |
```
...
If a teammate did not complete its screenshot upload (e.g., crashed mid-run), manually stage its `/tmp/` screenshots into the side branch using the flow in `.Codex/skills/testing-pack/references/pr-screenshot-upload.md`.
...
For each category in `enterprise_rag`, `enterprise_rag_aiq`, `paas_rag`, `cuopt`, `vss`, `warehouse_pick_path`, `dox_pack`:
...
```bash
# Delete old assets
for asset in enterprise_rag enterprise_rag_aiq paas_rag cuopt vss warehouse_pick_path dox_pack; do
...
| `doxPack.zip` | "Document Extractor Deployment Size" |
