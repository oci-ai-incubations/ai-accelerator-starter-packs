...
1. **Base coverage files** — pack-specific split test files in `.Codex/skills/<category>-test-coverage/`:
...
```
.Codex/skills/<category>-test-coverage/SKILL.md
.Codex/skills/<category>-test-coverage/api-tests.md
.Codex/skills/<category>-test-coverage/ui-tests.md
.Codex/skills/<category>-test-coverage/infra-tests.md
```

For example: `.Codex/skills/vss-test-coverage/`, `.Codex/skills/paas-rag-test-coverage/`, etc.
...
> **JUST-IN-TIME LOADING:** Read `.Codex/skills/<category>-test-coverage/api-tests.md` NOW. This file is self-contained — it has every API test with endpoint, method, request body, verification criteria, priorities, timeouts, and curl commands. Execute directly from it.
...
> **JUST-IN-TIME LOADING:** Read `.Codex/skills/<category>-test-coverage/infra-tests.md` NOW. This file is self-contained — it has every infrastructure test with kubectl/OCI CLI commands, expected output, and failure hints.
...
If the spec file does NOT exist for a category, fall back to reading `.Codex/skills/<category>-test-coverage/ui-tests.md` and generating a temporary spec file following the same patterns as `tests/e2e/vss/vss.spec.ts`.
...
═══════════════════════════════════════════════════
  ...
  BASE SPEC (.Codex/skills/<category>-test-coverage/):
...
mkdir -p .Codex/test-artifacts

# Copy report
cp "${DAT_SANDBOX}/logs/test-report.txt" \
   ".Codex/test-artifacts/<category>-<size>-test-report-<YYYY-MM-DD>.txt"

# Copy UI recordings from Playwright test-results
cp tests/e2e/test-results/<category>-recording/*.webm \
   ".Codex/test-artifacts/<category>-<size>-base-recording.webm" 2>/dev/null || true
cp tests/e2e/test-results/<category>-custom-recording/*.webm \
   ".Codex/test-artifacts/<category>-<size>-custom-recording.webm" 2>/dev/null || true

# Copy API results from sandbox
cp ${DAT_SANDBOX}/api-results/*.json \
   ".Codex/test-artifacts/" 2>/dev/null || true
```

The `.Codex/test-artifacts/` directory is gitignored — artifacts are for local review only, not committed.
...
Tell the user: *"Test artifacts have been copied to `.Codex/test-artifacts/`. The sandbox `${DAT_SANDBOX}/` will persist until you delete it."*
