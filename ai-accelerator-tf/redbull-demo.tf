...
# Shared secret for the rag-ingestor service account. Generated once and fed to
# BOTH the auth-service (AUTH_BOOTSTRAP_CLIENT_SECRET, which bcrypt-hashes + seeds
# it) and the ingestor (AUTH_CLIENT_SECRET via etl-secrets). `special=false` keeps
# it form/URL-safe for the client_credentials POST body.
resource "random_password" "rag_ingestor_client_secret" {
  length  = 48
  special = false
}
...
locals {
  ...
  rag_ingestor_image_uri = "ord.ocir.io/iduyx1qnmway/corrino-devops-repository/paas-rag-ingestor:ee697de-1"

  # Service-account (machine identity) the rag-ingestor uses to obtain a
  # llama-stack JWT via the OAuth2 client_credentials grant — no human login.
  # The auth-service env-seeds this client_id + secret on startup
  # (AUTH_BOOTSTRAP_CLIENT_*); the ingestor reads the same creds from etl-secrets
  # / etl-config. Stable client_id keeps OGX resource ownership consistent.
  rag_ingestor_client_id = "cli_rag_ingestor"
...
resource "kubernetes_secret_v1" "etl_secrets" {
  ...
  data = {
    ...
    AUTH_CLIENT_SECRET = random_password.rag_ingestor_client_secret.result
  }
...
resource "kubernetes_config_map_v1" "etl_config" {
  ...
  data = {
    ...
    AUTH_CLIENT_ID   = local.rag_ingestor_client_id
