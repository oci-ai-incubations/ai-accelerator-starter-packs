...
resource "oci_identity_policy" "operator_policy" {
  ...
  compartment_id = local.tenancy_ocid
  statements = [
    "Allow dynamic-group 'operator_dg-${random_string.deploy_id.result}' to manage cluster-family in compartment id ${local.compartment_ocid}"
  ]
  count = local.create_network_resources && local.create_bastion_effective && var.create_policies ? 1 : 0
  depends_on = [
...
resource "oci_identity_policy" "oke_instances_tenancy_policy" {
  ...
  statements = [
    "Allow dynamic-group ${oci_identity_dynamic_group.dyn_group[0].name} to manage all-resources in compartment id ${local.compartment_ocid}",
    "Allow dynamic-group ${oci_identity_dynamic_group.dyn_group[0].name} to use all-resources in tenancy",
    "Allow dynamic-group ${oci_identity_dynamic_group.dyn_group[0].name} to manage cluster-node-pools in compartment id ${local.compartment_ocid}",
    "Allow dynamic-group ${oci_identity_dynamic_group.dyn_group[0].name} to manage cluster-work-requests in compartment id ${local.compartment_ocid}",
