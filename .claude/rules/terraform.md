...
- Starter pack categories are: `enterprise_rag`, `paas_rag`, `cuopt`, `vss`. Adding a new category requires changes in `vars.tf`, `app-locals.tf`, `blueprint_files.tf`, a new schema YAML, `create_final_schema.py`, schema expectations, and a new test file.
