...
| Repeated apply failures after cleanup attempts (multiple `already exists` / `cannot re-use` / `installation failed` / timeouts in succession) | Cluster is in a bad state from cascading partial deploys. Manual cleanup of individual resources creates more problems (orphaned CRDs, dangling PVCs, stuck finalizers, stale state). This applies to any resource type — Helm releases, K8s secrets, configmaps, blueprints, etc. | **Stop trying to fix individual resources.** Destroy both the app AND infra stacks completely, delete them, and start fresh. A clean start is more reliable than manually cleaning up partial state, even though bare-metal GPU reprovisioning can take up to 6 hours. Destroy order: app stack first, then infra stack. |
...
| Customer secret key quota exceeded (create fails with limit error) | Each OCI user can have max 2 customer secret keys. The `paas_rag` pack creates one for object storage access. Repeated deploys/destroys accumulate keys. | Delete existing keys: `oci iam customer-secret-key list --user-id <user_ocid>` then `oci iam customer-secret-key delete --user-id <user_ocid> --customer-secret-key-id <key_id> --force`. Always clean up keys after destroying stacks that create them. |
| `Out of host capacity` (instance pool work request) | No bare-metal GPU hosts available in the target region/AD. The instance pool retries with backoff but capacity may not free up for hours. **This error does NOT appear in Terraform logs** — you must check instance pool work requests via OCI CLI. | Check work requests: `oci work-requests work-request list --compartment-id <OCID> --resource-id <POOL_OCID>` then `oci work-requests work-request-error list --work-request-id <WR_OCID>`. Options: wait for retry, try a different region, or try a smaller GPU shape. Also check the shape requested: `oci compute-management instance-configuration get --instance-configuration-id <CONFIG_OCID> --query 'data."instance-details"."launch-details".shape'`. |
...
| `secrets ".*" already exists` / `configmaps ".*" already exists` / `persistentvolumeclaims ".*" already exists` | Orphaned Kubernetes resources from a previous app stack that was deleted without running Destroy first. The ORM stack was removed but its resources remain on the cluster. A new app stack tries to create them and fails. | **Root cause: the previous app stack was deleted without Destroy.** Fix: clean up orphaned resources manually via kubectl (`kubectl delete secret/configmap/pvc <name> -n <namespace>`), also `helm uninstall` any orphaned Helm releases. Then re-run apply. **Prevention: ALWAYS run ORM Destroy on an app stack before deleting it.** |
