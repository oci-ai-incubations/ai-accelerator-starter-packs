...
Validate, rename, announce, and publish a release after the build phase (RELEASE_BUILD.md) has completed.

## Arguments

- **VERSION** — Passed from the calling skill (SKILL.md). Semver format (e.g., `v0.0.5`). If not available, read from `ai-accelerator-tf/AI_ACCELERATOR_STACK_VERSION`.

## Prerequisites

1. Confirm the build phase (RELEASE_BUILD.md) has already been run — check that a "Release \<VERSION\>" commit exists on the current branch, and the necessary files are present to build the Slack post
